...
abstract class BaseUnit<T : BaseUnit<T>>(
     ...
     * 반올림 적용
     */
    private fun applyRounding(value: Double, digits: Int): Double {
        val factor = 10.0.pow(digits)
        return when (roundingMode) {
            RoundingMode.HALF_UP -> kotlin.math.round(value * factor) / factor
            RoundingMode.HALF_DOWN -> kotlin.math.floor(value * factor + 0.5) / factor
            RoundingMode.HALF_EVEN -> {
                ...
                if (kotlin.math.abs(scaled - rounded) == 0.5) {
                    ...
                    else kotlin.math.floor(scaled) / factor
                } else rounded / factor
            }
            RoundingMode.UP -> kotlin.math.ceil(value * factor) / factor
            RoundingMode.DOWN -> kotlin.math.floor(value * factor) / factor
        }
    }
